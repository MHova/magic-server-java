...
public class BoardsResource {
	...
	public BoardsResource(final MongoClient mongoClient) {
	...
	public Response populateDummyData() throws URISyntaxException {
		...
		db.getCollection("boards", TwoPlayerBoard.class)
				.insertOne(new TwoPlayerBoard("one", player1, player2));

		return Response.created(new URI("/boards/one")).build();
	}
	...
	public TwoPlayerBoard getBoard(
			@PathParam("boardId") Optional<String> maybeBoardId) {
		final String boardId = maybeBoardId.orElseThrow(
				() -> new BadRequestException("board ID is required"));
		FindIterable<TwoPlayerBoard> findIterable = db
				.getCollection("boards", TwoPlayerBoard.class)
				.find(Filters.eq(boardId));

		final Optional<TwoPlayerBoard> maybeRetVal = Optional
				.ofNullable(findIterable.first());
		return maybeRetVal
				.orElseThrow(() -> new NotFoundException("No such board"));
	}
}
